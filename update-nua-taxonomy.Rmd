...
source("code/02_filter_taxa_worms.R")
```
...
start.time <- Sys.time()
source("code/03_get_worms_synonyms.R")
end.time <- Sys.time()
...
start.time <- Sys.time()
source("code/04_get_dyntaxa_links.R")
end.time <- Sys.time()
...
start.time <- Sys.time()
source("code/05_get_algaebase_links.R")
end.time <- Sys.time()
...
```{r wrangle_norcca, echo=FALSE, message=FALSE}
if(!params$cache_norcca) {
  file.remove("norcca_cache.rda")
}

source("code/06_match_norcca_names.R")
end.time <- Sys.time()
...
```{r wrangle_hab_list, echo=FALSE, include=FALSE}
source("code/07_get_hab_links.R")
```
...
if(!params$cache_itis) {
  file.remove("itis_cache.rda")
}

start.time <- Sys.time()
source("code/08_get_itis_links.R")
end.time <- Sys.time()
...
if(!params$cache_ncbi) {
  file.remove("ncbi_cache.rda")
}

start.time <- Sys.time()
source("code/09_get_ncbi_links.R")
end.time <- Sys.time()
...
if(!params$cache_gbif) {
  file.remove("gbif_cache.rda")
}

start.time <- Sys.time()
source("code/10_get_gbif_links.R")
end.time <- Sys.time()
...
source("code/11_update_filters.R")
```
