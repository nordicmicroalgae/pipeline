...
if(!params$cache_taxa_worms & file.exists("cache/worms_cache.db")) {
...
# Find taxa with duplicated AlgaeBase links

```{r duplicate_algaebase, echo=FALSE}
# Print table
algaebase_duplicates %>%
  rename(authority_ab = authorship) %>%
  select(taxon_id,
         ab_id,
         scientific_name,
         authority,
         authority_ab,
         url) %>%
  knitr::kable(caption = "Taxa with duplicated algaebase links")
```
