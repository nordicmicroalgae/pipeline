...
taxonomic.id<-algaebase_output_parse(results.output,"dwc:scientificNameID")
...
output<-data.frame(id=taxonomic.id,genus=taxonomic.genus,species=taxonomic.species,infrasp=NA,taxonomic.status,currently.accepted,genus.only=0,accepted.name,input.name=input.clean.name,
...
for(i in 1:nrow(output)){
  ...
  if(higher){ #merge higher taxonomy and reorder names of output variable
    ...
    output<-subset(output,select= c('id','accepted.name','input.name','input.match','currently.accepted','genus.only','kingdom','phylum','class','order','family','genus','species','infrasp',
                                    'long.name','taxonomic.status','taxon.rank','mod.date','authorship'))}else{
    output<-subset(output,select=c('id','accepted.name','input.name','input.match','currently.accepted','genus.only','genus','species','infrasp',
