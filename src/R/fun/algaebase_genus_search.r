...
algaebase_genus_search<-function(genus=NULL,apikey=NULL,handle=NULL,
  ...
  if(higher==TRUE){
    id<-algaebase_output_parse(results.output,"dwc:scientificNameID")
  ...
  output<-data.frame(id=id,genus=taxonomic.genus,species=NA,infrasp=NA,taxonomic.status,currently.accepted,accepted.name,genus.only=1,input.name=genus,
